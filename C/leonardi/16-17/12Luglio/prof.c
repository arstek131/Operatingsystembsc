		...
		if (pid == 0) 
		{
			...
			if (pid == 0) 
			{
				...
				if (S.linea_letta[j] == '\n')  
					...
					{ ritorno++; 
					/* nell'array linea_letta abbiamo l'ultima linea ricevuta: incrementiamo la sua lunghezza per inserire il terminatore di stringa che serve solo per poter usare le funzioni delle stringhe (da parte del padre), altrimenti si puo' non farlo */
					j++;		
					S.linea_letta[j]='\0';
