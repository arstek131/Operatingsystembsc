        ...
        for (i=0; i < N; i++)
        {
                ...
                if (pid == 0)
                {
                        ...
                        if (pid == 0)
                        {
                                ...
                                /* printf("Sono il processo nipote del figlio di indice %d e pid %d sto per eseguire il comando sort -f per il file %s\n", i, getpid(), argv[(i*2)+1]); */
                                ...
                                sprintf(opzione, "-%c", 'f');
                                /* stampa di controllo */
                                /* printf("Sono il processo nipote di indice %d e pid %d sto per fare exec di sort del file %s con opzione %s\n", i, getpid(), argv[i+1], opzione); */
                                ...
                                /* attenzione ai parametri nella esecuzione di sort: opzione, nome del file e terminatore della lista */
                                ...
                                while (read(p[0], &(S.linea_letta[j]), 1))
                                {
                                	...
                                	if (S.linea_letta[j] == '\n')
                                	{       
					   	/* aggiorniamo comunque il campo con la lunghezza in caratteri della linea */
						S.lung_linea=j+1; /* la lunghezza della linea compreso il terminatore e' j+1 (se fosse richiesta la lunghezza della linea non compreso il terminatore, sarebbe solo j) */	
                                        	/* nell'array linea_letta abbiamo l'ultima linea ricevutaa */
						...
						/* N.B. Non serve azzerare j dato che usciamo dopo la lettura della prima linea */
                                ...
                                if ((status & 0xFF) != 0)
                                {
                                        ...
                                        /* printf("Il nipote con pid=%d ha ritornat31Mag19o %d\n", pid, (int)((status >> 8) & 0xFF)); */
				/* il figlio ritorna la lunghezza della linea inviata NON compreso il terminatore (se fosse compreso il terminatore il valore di lung_linea andrebbe incrementato */
...
/* Il padre recupera le informazioni dai figli: in ordine di indice */
        for (i=0; i<N; i++)
        {
         /* si legge la struttura inviata  dal figlio i-esimo */
            ...
            if (nr != 0)
            {
            	/* Nota bene: la stampa della linea con il formato %s richiede che il padre inserisca il terminatore di stringa nel posto giusto, ad esempio al posto del terminatore di linea */
